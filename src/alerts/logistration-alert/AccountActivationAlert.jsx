...
const AccountActivationAlert = () => {
    ...
    const button = (
        <Button variant="primary" className="" onClick={() => setShowModal(false)}>
            <FormattedMessage
    ...
    const children = () => {
        ...
        const message = (
            <FormattedMessage
                ...
                defaultMessage="Chúng tôi đã gửi email đến {boldEmail} với liên kết để kích hoạt tài khoản của bạn. Không tìm thấy? Kiểm tra thư mục spam hoặc {sendEmailTag}."
        ...
        if (!showCheck && showSpinner) {
            bodyContent = (
                <div>
                    {message}
                    <Spinner animation="border" variant="secondary" style={{ height: "1.5rem", width: "1.5rem" }} />
                </div>
            );
        }
