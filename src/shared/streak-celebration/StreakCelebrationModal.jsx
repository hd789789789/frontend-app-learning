...
function getRandomFactoid(intl, streakLength) {
    ...
    const factoids = [
        <FormattedMessage
            ...
            defaultMessage="Người dùng học {streak_length} ngày liên tiếp {bolded_section} so với những người không làm vậy."
            values={{
...
const StreakModal = ({
    ...
    useEffect(() => {
        if (streakDiscountCouponEnabled && verifiedMode && getConfig().ECOMMERCE_BASE_URL) {
            calculateVoucherDiscount(discountCode, verifiedMode.sku, username).then(
                (result) => {
    ...
    if (verifiedMode) {
        ...
        if (discountPercent > 0) {
            ...
            offer = {
                discountedPrice: `${verifiedMode.currencySymbol}${(mode.price * discountMultipler)
                    .toFixed(2)
                    .toString()}`,
    ...
    return (
        ...
        >
            <ModalDialog.Header className="modal-header">
                <ModalDialog.Title className="mr-0 modal-title">{title}</ModalDialog.Title>
            </ModalDialog.Header>
            <ModalDialog.Body className="modal-body">
                ...
                {queryingDiscount && <Spinner animation="border" variant="primary" />}
                {!queryingDiscount && !showOffer && (
                    <div className="d-flex py-3 bg-light-300">
                        <Icon className="col-small ml-3" src={Lightbulb} />
                        <div className="col-11 factoid-wrapper">{randomFactoid}</div>
                    </div>
                )}
                {!queryingDiscount && showOffer && (
            ...
            <ModalDialog.Footer className="modal-footer d-block">
                ...
                {!queryingDiscount && !showOffer && (
                    <ModalDialog.CloseButton className="px-5" variant="primary">
                        <CloseText intl={intl} />
                    </ModalDialog.CloseButton>
                )}
            </ModalDialog.Footer>
