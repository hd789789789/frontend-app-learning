...
const CourseCelebration = () => {
    ...
    switch (certStatus) {
            ...
            message = (
                <p>
                    <FormattedMessage
                        ...
                        defaultMessage="Thể hiện thành tích của bạn trên LinkedIn hoặc sơ yếu lý lịch của bạn ngay hôm nay. Bạn có thể tải xuống chứng chỉ của mình ngay bây giờ và truy cập bất kỳ lúc nào từ {dashboardLink} và {profileLink} của bạn."
                ...
                <>
                    <p>
                        <FormattedMessage
                            ...
                            defaultMessage="Khóa học này kết thúc vào {endDate}. Điểm cuối kỳ và bất kỳ chứng chỉ nào đã đạt được dự kiến sẽ có sẵn sau {certAvailableDate}."
                    ...
                    <p>{intl.formatMessage(messages.certificateNotAvailableBodyAccessCert)}</p>
                </>
            );
                ...
                message = (
                    <p>
                        <FormattedMessage
                            ...
                            defaultMessage="Để tạo chứng chỉ, bạn phải hoàn thành xác minh danh tính. {idVerificationSupportLink} ngay bây giờ."
        ...
        case "honor_passing":
            if (verifiedMode) {
                ...
                message = (
                    <p>
                        <FormattedMessage
                            ...
                            defaultMessage="Vẫn chưa quá muộn để nâng cấp. Với {price} bạn sẽ mở khóa quyền truy cập vào tất cả các bài tập đã chấm điểm trong khóa học này. Sau khi hoàn thành, bạn sẽ nhận được chứng chỉ đã xác minh là một thông tin xác thực có giá trị để cải thiện triển vọng nghề nghiệp và thăng tiến trong sự nghiệp của bạn, hoặc làm nổi bật chứng chỉ của bạn trong các đơn xin học."
                ...
                if (verifiedMode.accessExpirationDate) {
                    footnote = (
                        <UpgradeFootnote deadline={verifiedMode.accessExpirationDate} href={verifiedMode.upgradeUrl} />
                    );
                } else {
                    footnote = <DashboardFootnote variant={visitEvent} />;
                }
        ...
        default:
            if (!canViewCertificate) {
                ...
                message = (
                    <>
                        <p>{intl.formatMessage(messages.certificateNotAvailableEndDateBody, { endDate })}</p>
                        <p>{intl.formatMessage(messages.certificateNotAvailableBodyAccessCert)}</p>
                    </>
                );
            }
    ...
    return (
        <>
            <Helmet>
                <title>{`${intl.formatMessage(messages.congratulationsHeader)} | ${title} | ${
                    getConfig().SITE_NAME
                }`}</title>
            </Helmet>
            <div className="row w-100 mx-0 mb-4 px-5 py-4 border border-light">
                <div className="col-12 p-0 h2 text-center">{intl.formatMessage(messages.congratulationsHeader)}</div>
