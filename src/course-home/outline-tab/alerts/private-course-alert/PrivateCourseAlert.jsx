...
const PrivateCourseAlert = ({ payload }) => {
    ...
    const signIn = (
        <Hyperlink style={{ textDecoration: "underline" }} destination={`${getLoginRedirectUrl(global.location.href)}`}>
    ...
    return (
        <Alert variant="light" data-testid="private-course-alert">
            {anonymousUser && (
                <>
                    <p className="font-weight-bold">{intl.formatMessage(enrollmentMessages.alert)}</p>
                    <FormattedMessage
                        id="learning.privateCourse.signInOrRegister"
                        description="Prompts the user to sign in or register to see course content."
                        defaultMessage="{signIn} hoặc {register} và sau đó ghi danh vào khóa học này."
                        values={{
                            signIn,
                            register,
                        }}
            ...
            {!anonymousUser && (
                <>
                    ...
                    {!canEnroll && <>{intl.formatMessage(enrollmentMessages.alert)}</>}
                </>
            )}
        </Alert>
    );
